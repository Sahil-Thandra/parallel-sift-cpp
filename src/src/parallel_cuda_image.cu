...
__global__ void hostToDeviceImageCopy(const Image& hostimg, Image* devimg){
    if (threadIdx.x == 0 && blockIdx.x == 0)
    {
        for (int k = 0; k < devimg->size; k++) {
            devimg->data[k] = hostimg.data[k];
        }
    }
}

__global__ void deviceToHostImageCopy(const Image& hostimg, Image* devimg){
    if (threadIdx.x == 0 && blockIdx.x == 0)
    {
        for (int k = 0; k < devimg->size; k++) {
            hostimg.data[k] = devimg->data[k];
        }
    }
}
...
Image gaussian_blur(const Image& img, float sigma)
{
    ...
    cudaMalloc((void**)&dev_img, sizeof(Image));
    setDeviceImageSize<<<1,1>>>(dev_img, img.width, img.height, img.channels);
    cudaDeviceSynchronize();

    cudaMalloc((void**)&dev_img_data, img.size * sizeof(float));
    cudaMemcpy(&(dev_img->data), &dev_img_data, sizeof(float*), cudaMemcpyHostToDevice);
    hostToDeviceImageCopy<<<1, 1>>>(img, dev_img);
    cudaDeviceSynchronize();
    ...
    cudaMemcpy(&(dev_kernel->data), &dev_kernel_data, sizeof(float*), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(&(dev_temp->data), &dev_temp_data, sizeof(float*), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(&(dev_filtered->data), &dev_filtered_data, sizeof(float*), cudaMemcpyHostToDevice);
    ...
    // convolve horizontal
    convolve_horizontal<<<numBlocks, threadsPerBlock>>>(*dev_img, *dev_kernel, size, center, *dev_temp);
    cudaDeviceSynchronize();

    deviceToHostImageCopy<<<1,1>>>(filtered, dev_filtered);
    cudaDeviceSynchronize();

    cudaFree(&dev_img->data);
    cudaFree(&dev_img);
    cudaFree(&dev_kernel->data);
    cudaFree(&dev_kernel);
    cudaFree(&dev_temp->data);
    cudaFree(&dev_temp);
    cudaFree(&dev_filtered->data);
